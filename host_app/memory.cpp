...
void write_io_register(uint32_t address, uint32_t value) {
 ...
 void write(uint32_t address, T value) {
    ...
    if (address == 0x309720) {
     ...
     // Watch the streaming state at param_1+0x8E8 (0x4202e0 + 0x8E8 = 0x420BC8)
     if(address >= 0x420BC8 && address < 0x420BD0){
         g_logFile << "[STREAM-STATE-WATCH] Write<" << sizeof(T) << "> addr=0x" << std::hex << address
                 << " value=0x" << (uint64_t)value
                 << " PC=0x" << g_current_pc << std::dec << std::endl;
     }
 ...
 void write_quad(uint32_t address, const QuadWord& value) {
     ...
     // Watch streaming state at 0x420BC8
     if (address <= 0x420BC8 && (address + 16) > 0x420BC8) {
         g_logFile << "[STREAM-STATE-WATCH] WriteQuad addr=0x" << std::hex << address
                   << " values=0x" << value.u32[0] << " 0x" << value.u32[1]
                   << " 0x" << value.u32[2] << " 0x" << value.u32[3]
                   << " PC=0x" << g_current_pc << std::dec << std::endl;
     }
