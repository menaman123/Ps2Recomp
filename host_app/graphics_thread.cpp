...
// Create the renderer on THIS thread — SDL2 GPU contexts are thread-local.
// Creating on the main thread and rendering here causes silent failures.
if (g_window && !g_renderer) {
  g_renderer = SDL_CreateRenderer(g_window, -1, SDL_RENDERER_ACCELERATED);
  if (!g_renderer) {
      g_logFile << "[GFX] SDL_CreateRenderer failed: " << SDL_GetError() << std::endl;
      return;
  }
  g_logFile << "[GFX] Renderer created on graphics thread." << std::endl;
}
...
switch (job.type) {
   case RenderCommandType::VSync:
   {
       ...
       // === VRAM SCANOUT: Read framebuffer from gs_vram at DISPFB address ===
       // On the real PS2, the display circuit reads from VRAM at the DISPFB address.
       // Since our draw commands don't rasterize into gs_vram, we at least show
       // whatever has been written via IMAGE mode (texture uploads, screen clears).
       {
           uint64_t dispfb = g_gs_regs.DISPFB2 ? g_gs_regs.DISPFB2 : g_gs_regs.DISPFB1;
           uint32_t fbp = (dispfb & 0x1FF) * 2048;  // FBP field * 2048 = byte address
           uint32_t fbw = ((dispfb >> 9) & 0x3F) * 64; // FBW field * 64 = pixel width
           uint32_t psm = (dispfb >> 15) & 0x1F;


           int scanW = (fbw > 0) ? fbw : 640;
           int scanH = windowH > 0 ? windowH : 448;
           if (scanW > 1024) scanW = 1024;
           if (scanH > 1024) scanH = 1024;


           static SDL_Texture* scanout_tex = nullptr;
           static int last_w = 0, last_h = 0;


           if (scanW > 0 && scanH > 0 && fbp < gs_vram.size()) {
               // Recreate texture if dimensions changed
               if (!scanout_tex || last_w != scanW || last_h != scanH) {
                   if (scanout_tex) SDL_DestroyTexture(scanout_tex);
                   scanout_tex = SDL_CreateTexture(g_renderer,
               ...
               if (scanout_tex) {
                   // Decode VRAM framebuffer to RGBA
                   std::vector<uint32_t> pixels(scanW * scanH);
                   for (int y = 0; y < scanH; y++) {
                       for (int x = 0; x < scanW; x++) {
                           uint32_t addr = fbp + (y * fbw + x) * 4; // PSMCT32 linear
                           if (psm == 0x02) { // PSMCT16
                               addr = fbp + (y * fbw + x) * 2;
                           }
                           if (addr + 3 < gs_vram.size()) {
                               if (psm == 0x00 || psm == 0x01) { // PSMCT32 or PSMCT24
                                   uint8_t r = gs_vram[addr + 0];
                                   uint8_t g = gs_vram[addr + 1];
                                   uint8_t b = gs_vram[addr + 2];
                                   uint8_t a = (psm == 0x01) ? 0xFF : gs_vram[addr + 3];
                                   // PS2 alpha: 0x80 = fully opaque, scale to 0xFF
                                   if (psm == 0x00) a = (uint8_t)std::min((int)a * 2, 255);
                                   pixels[y * scanW + x] = r | (g << 8) | (b << 16) | (a << 24);
                               } else if (psm == 0x02) { // PSMCT16
                                   uint16_t px = gs_vram[addr] | (gs_vram[addr+1] << 8);
                                   uint8_t r = (px & 0x1F) << 3;
                                   uint8_t g2 = ((px >> 5) & 0x1F) << 3;
                                   uint8_t b = ((px >> 10) & 0x1F) << 3;
                                   uint8_t a = (px & 0x8000) ? 0xFF : 0;
                                   pixels[y * scanW + x] = r | (g2 << 8) | (b << 16) | (a << 24);
                               } else {
                                   pixels[y * scanW + x] = 0xFF000000; // Unknown PSM: black opaque
                               }
                           }
                       }
                   }
                   ...
                   SDL_UpdateTexture(scanout_tex, nullptr, pixels.data(), scanW * 4);
                   SDL_SetTextureBlendMode(scanout_tex, SDL_BLENDMODE_NONE);
                   SDL_RenderCopy(g_renderer, scanout_tex, nullptr, nullptr);
               }
           }
       }
   ...
   case RenderCommandType::DrawBatch:
   {
           ...
           if (ti.psm >= 0x30 && ti.psm <= 0x3A) {
               ...
               bool pages_written = false;
               {
                   ...
                   switch (ti.psm) {
                       ...
                       case 0x01: tex_bytes = (size_t)tex_w * tex_h * 4; break; // PSMCT24 stored as 32bpp
       ...
       for (size_t i = 0; i < vCount; i++) {
           ...
           // DEBUG: Force all draws fully opaque to verify rendering pipeline works.
           // Remove this once fade-in timing is fixed.
           sv.color.a = 255;
       ...
       size_t tri_verts = (sdl_verts.size() / 3) * 3;
        if (tri_verts >= 3) {
           ...
           int rc = SDL_RenderGeometry(g_renderer,
                             sdl_tex,
                             sdl_verts.data(),
                             (int)tri_verts,
                             nullptr, 0);
           if (rc != 0) {
               g_logFile << "[SDL-DRAW] SDL_RenderGeometry FAILED: " << SDL_GetError() << std::endl;
           }
       }
