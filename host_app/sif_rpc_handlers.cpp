...
// Diagnostic: verify RAM content after HLE file reads
static void VerifyRamContent(uint32_t dest_addr, size_t bytes_read) {
 int non_zero = 0;
 size_t check_len = std::min(bytes_read, (size_t)16);
 g_logFile << "[HLE READ VERIFY] dest=0x" << std::hex << dest_addr << " first_16_bytes:";
 for (size_t i = 0; i < check_len; i++) {
     uint8_t b = memory::read<uint8_t>(dest_addr + i);
     g_logFile << " " << std::setw(2) << std::setfill('0') << (int)b;
     if (b != 0) non_zero++;
 }
 g_logFile << std::dec << std::endl;
 g_logFile << "[HLE READ VERIFY] non_zero_count=" << non_zero << "/" << check_len
           << " (data looks " << (non_zero > 0 ? "VALID" : "EMPTY") << ")" << std::endl;
}
// ============================================================================
// MASTER DISPATCHER
// ============================================================================
void DispatchSifRpcCall(const SifRpcContext& rpc) {
 ...
 // Basic I/O and System
     ...
     // CDVD DiskReady (sceCdDiskReady)
     case 0x8000059c: HandleCdDiskReady(rpc);     break;
...
switch (rpc.func_num) {
 ...
 case 0x06: {
     ...
     if (it != g_file_io_handles.end()) {
         ...
         VerifyRamContent(phys_addr, bytes);
     } else {
     ...
     if (g_isoFile) {
         ...
         if (iso_offset + size <= iso_size) {
             ...
             if (dest_ptr) {
                 ...
                 VerifyRamContent(phys_addr, bytes_read);
             } else {
...
void HandleCustomLoader(const SifRpcContext& rpc) {
 ...
 // Verbose: dump ALL payload data regardless of func_num
 g_logFile << "    [CustomLoader-VERBOSE] func=" << rpc.func_num
           << " payload_addr=0x" << std::hex << rpc.payload_addr
           << " payload_size=" << std::dec << rpc.payload_size
           << " recv_buffer=0x" << std::hex << rpc.recv_buffer
           << " recv_size=" << std::dec << rpc.recv_size << std::endl;
 if (rpc.payload_addr != 0) {
     int dump_len = (rpc.payload_size < 64) ? 64 : rpc.payload_size;
     g_logFile << "    [CustomLoader-VERBOSE] Payload dump:" << std::endl;
     for (int i = 0; i < dump_len; i += 4) {
         uint32_t val = memory::read<uint32_t>(rpc.payload_addr + i);
         g_logFile << "      [+" << std::hex << i << "] = 0x" << val << std::dec << std::endl;
     }
 }
     ...
     if (g_isoFile) {
         ...
         if (iso_offset + size <= iso_size) {
             ...
             if (dest_ptr) {
                 ...
                 VerifyRamContent(dest_addr, bytes_read);
             } else {
...
// ============================================================================
// CDVD DISK READY (sceCdDiskReady) - Server 0x8000059C
// ============================================================================
// This is the CdDiskReady RPC server from libcdvd.
// The game calls this to check if the disc is ready for reading.
//
// Func 0: DiskReady check
//   Payload (4 bytes): mode (0 = blocking wait, 8 = non-blocking poll)
//   Response (4 bytes): disc status
//     0 = SCECdNotReady
//     2 = SCECdComplete (disc ready)
//     6 = SCECdNV (no disc)
// ============================================================================
void HandleCdDiskReady(const SifRpcContext& rpc) {
   uint32_t mode = 0;
   if (rpc.payload_addr != 0 && rpc.payload_size >= 4) {
       mode = memory::read<uint32_t>(rpc.payload_addr);
   }
  
   g_logFile << "    [HLE] CdDiskReady (Func: 0x" << std::hex << rpc.func_num
             << ", Mode: 0x" << mode << ")" << std::dec << std::endl;
  
   // Always report disc as ready (SCECdComplete = 2)
   if (rpc.recv_buffer != 0 && rpc.recv_size >= 4) {
       memory::write<uint32_t>(rpc.recv_buffer, 2);  // SCECdComplete
       g_logFile << "    [HLE] CdDiskReady: Wrote SCECdComplete (2) to recv_buffer 0x"
                 << std::hex << rpc.recv_buffer << std::dec << std::endl;
   }
}
...
void HandleUnknownServer(const SifRpcContext& rpc) {
 ...
 g_logFile << "    [SIF-UNKNOWN] server_id=0x" << std::hex << rpc.server_id
           << " func_num=" << std::dec << rpc.func_num
           << " payload_addr=0x" << std::hex << rpc.payload_addr
           << " payload_size=" << std::dec << rpc.payload_size
           << " recv_buffer=0x" << std::hex << rpc.recv_buffer
           << " recv_size=" << std::dec << rpc.recv_size
           << " client_data_addr=0x" << std::hex << rpc.client_data_addr
           << std::dec << std::endl;
 // Dump payload bytes
 if (rpc.payload_addr != 0 && rpc.payload_size > 0) {
     int dump_len = (rpc.payload_size < 64) ? rpc.payload_size : 64;
     g_logFile << "    [SIF-UNKNOWN] Payload as uint32s:" << std::endl;
     for (int i = 0; i < dump_len + 32; i += 4) {
         uint32_t val = memory::read<uint32_t>(rpc.payload_addr + i);
         g_logFile << "      [+" << std::hex << i << "] = 0x" << val << std::dec << std::endl;
     }
     // Try reading as string from various offsets
     for (int off = 0; off < 48; off += 4) {
         std::string str = ReadGuestString(rpc.payload_addr + off, 64);
         if (!str.empty() && str[0] >= 0x20 && str.length() > 2) {
             g_logFile << "      [+" << std::hex << off << " str] \"" << str << "\"" << std::dec << std::endl;
         }
     }
 }
 // Dump recv_buffer current contents before we write
 if (rpc.recv_buffer != 0 && rpc.recv_size > 0) {
     int dump_len = (rpc.recv_size < 32) ? rpc.recv_size : 32;
     g_logFile << "    [SIF-UNKNOWN] recv_buffer BEFORE (at 0x" << std::hex << rpc.recv_buffer << "):";
     for (int i = 0; i < dump_len; i += 4) {
         uint32_t val = memory::read<uint32_t>(rpc.recv_buffer + i);
         g_logFile << " 0x" << std::hex << val;
     }
     g_logFile << std::dec << std::endl;
 }
 // Best-effort response: write 0 to recv_buffer
 WriteResult32(rpc.recv_buffer, 0);
 g_logFile << "    [SIF-UNKNOWN] Wrote 0 to recv_buffer" << std::endl;
}