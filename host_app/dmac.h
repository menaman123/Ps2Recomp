...
public:
   ...
   // Suspended DMA chain support (for async chain building)
   void ResumeSuspendedDma();
   bool HasSuspendedDma() const;


   // Suspended VIF DMA chain state
   struct SuspendedVifChain {
       bool active = false;
       int channel = 0;
       uint32_t tadr = 0;
       bool fromSpr = false;
       int tag_count = 0;
       int retry_count = 0;
       static constexpr int MAX_RETRIES = 120; // ~2 seconds at 60fps
   } suspended_vif_chain;
