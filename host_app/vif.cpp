...
void VIF::ProcessFifo(int v) {
  ...
  // All data consumed and no pending state — VIF is idle
  if (state[v] == VIF_STATE_IDLE) {
      UpdateStat(v, VIF_STAT::VPS_IDLE);
  }
}
...
void VIF::ExecuteUnpack(int v, uint32_t cmd, uint32_t immediate, uint32_t num,
  ...
  const uint8_t* src_end = payload.data() + payload.size();
   // [VIF-FIX] Diagnostic logging at UNPACK entry
  g_logFile << "[VIF-FIX] UNPACK entry: CMD=0x" << std::hex << cmd
            << " NUM=" << std::dec << num
  ...
  uint32_t write_cycle = 0;  // Cycles through 0 to WL-1 for mask rows
   // RC1 FIX: Remove src_ptr < src_end from loop condition.
  // Skip cycles (cycle_pos >= wl) do NOT consume source data, so the loop
  // must run all vectors_to_unpack iterations. Source bounds are checked
  // individually before each read inside write cycles only.
  for (uint32_t i = 0; i < vectors_to_unpack; i++) {
      ...
      // [VIF-FIX] Per-vector diagnostic logging
      g_logFile << "[VIF-FIX] vec " << i
                << " cycle_pos=" << cycle_pos
                << " do_write=" << do_write
                << " src_offset=" << (src_ptr - payload.data()) << std::endl;




      if (!do_write) {
          // Skip cycle: advance destination address only, do NOT read source data
          addr_qw = (addr_qw + 1) & 0x3FF;
          continue;
      }
    
      // Write cycle: read source data into temporary vector
   ...
   // [VIF-FIX] Diagnostic logging at UNPACK completion
  size_t consumed = static_cast<size_t>(src_ptr - payload.data());
  size_t expected = payload.size();
  g_logFile << "[VIF-FIX] UNPACK complete: consumed=" << consumed
            << " expected=" << expected
            << " vectors=" << vectors_to_unpack
            << (consumed == expected ? " OK" : " MISMATCH") << std::endl;
}
