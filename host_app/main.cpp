...
int main(int argc, char* argv[]) {
  ...
  // [DIAG] Populate watched functions for texture upload pipeline investigation
  g_watched_functions.insert(0x172b60);  // Texture load request (calls 2afb18)
  g_watched_functions.insert(0x2afb18);  // Initiates texture upload
  g_watched_functions.insert(0x2af0d8);  // Texture state machine
  g_watched_functions.insert(0x2ae318);  // Texture upload with rendering
  g_watched_functions.insert(0x2aea08);  // Standalone texture upload
  g_watched_functions.insert(0x2b0e50);  // BITBLTBUF setup trigger
  g_watched_functions.insert(0x177168);  // Main scene dispatcher
  g_logFile << "[DIAG] Watching " << g_watched_functions.size() << " functions for texture pipeline" << std::endl;
  ...
  g_window = SDL_CreateWindow(
   ...
   // NOTE: Renderer is created on the graphics thread (GraphicsThreadFunc)
  // because SDL2 GPU contexts are thread-local. Creating it here and using
  // it from another thread causes silent rendering failures on many drivers.
