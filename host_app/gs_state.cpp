...
// [GS-XFER] Session-wide transfer counters for diagnostic
uint32_t g_bitbltbuf_count = 0;
uint32_t g_trxdir_count = 0;
uint32_t g_hwreg_count = 0;
...
void GSState::SetBitBltBuf(uint64_t value) {
 ...
 transfer.dpsm = (value >> 56) & 0x3F;             // Dest format
  ...
  g_bitbltbuf_count++;
  if (g_logFile.is_open()) {
     g_logFile << "[GS-XFER] BITBLTBUF #" << g_bitbltbuf_count
               << ": SRC=0x" << std::hex << transfer.sbp
               << " → DST=0x" << transfer.dbp
               << " (PSM=" << std::dec << transfer.dpsm << ")" << std::endl;
 }
}
...
void GSState::SetTrxPos(uint64_t value) {
 ...
 transfer.dir  = (value >> 59) & 0x3;
  if (g_logFile.is_open()) {
     g_logFile << "[GS-XFER] TRXPOS: SRC(" << transfer.ssax << "," << transfer.ssay << ")"
...
void GSState::SetTrxReg(uint64_t value) {
 ...
 transfer.rrh = (value >> 32) & 0xFFF;
  if (g_logFile.is_open()) {
     g_logFile << "[GS-XFER] TRXREG: " << transfer.rrw << "x" << transfer.rrh
...
void GSState::SetTrxDir(uint64_t value) {
 transfer.xdir = value & 0x3;
  ...
  g_trxdir_count++;
  if (g_logFile.is_open()) {
     g_logFile << "[GS-XFER] TRXDIR #" << g_trxdir_count << ": "
               << dir_names[transfer.xdir]
               << " (transfer_count=" << g_trxdir_count << ")" << std::endl;
  
     if (transfer.xdir == 0) {
         g_logFile << "[GS-XFER] *** TEXTURE UPLOAD STARTED ***" << std::endl;
         g_logFile << "[GS-XFER]     Dest VRAM: 0x" << std::hex << transfer.dbp << std::dec << std::endl;
         g_logFile << "[GS-XFER]     Position: (" << transfer.dsax << "," << transfer.dsay << ")" << std::endl;
         g_logFile << "[GS-XFER]     Size: " << transfer.rrw << "x" << transfer.rrh << std::endl;
         g_logFile << "[GS-XFER]     Format: PSM=" << transfer.dpsm << std::endl;
     }
 }
...
void GSState::WriteHWReg(uint64_t value) {
 if (!transfer.active || transfer.xdir != 0) return;
  ...
  g_hwreg_count++;
  if (g_hwreg_count <= 5 || (g_hwreg_count % 10000 == 0)) {
      if (g_logFile.is_open()) {
          g_logFile << "[GS-XFER] HWREG write #" << g_hwreg_count
                    << ": value=0x" << std::hex << value << std::dec
                    << " (pixels_written=" << transfer.pixels_written
 ...
 switch (transfer.dpsm) {
  ...
  for (int p = 0; p < pixels_per_write; p++) {
     ...
     if (transfer.cur_y >= transfer.dsay + transfer.rrh) {
         if (transfer.active) {
             g_logFile << "[GS-XFER] *** TEXTURE UPLOAD COMPLETE ***" << std::endl;
             g_logFile << "[GS-XFER]     Pixels written: " << transfer.pixels_written << std::endl;


             // Sample first 16 pixels at transfer.dbp for diagnostic
             int non_zero_sample = 0;
             int pixel_size = GetPixelSize(transfer.dpsm);
             for (int si = 0; si < 16; si++) {
                 uint32_t sample_addr = transfer.dbp + si * pixel_size;
                 if (sample_addr + pixel_size <= VRAM_SIZE) {
                     for (int sb = 0; sb < pixel_size; sb++) {
                         if (gs_vram[sample_addr + sb] != 0) { non_zero_sample++; break; }
                     }
                 }
             }
             g_logFile << "[GS-XFER] TRANSFER COMPLETE: dest=0x" << std::hex << transfer.dbp
                       << std::dec << " pixels=" << transfer.pixels_written
                       << " non_zero_sample=" << non_zero_sample << "/16" << std::endl;
