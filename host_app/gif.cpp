...
// RC5 FIX: Context 0 fallback to context 1 when uninitialized
if (ctx == 0 && !g_gs_state.prmodecont && g_gs_state.frame[0].fbw == 0) {
 g_logFile << "[CTX-FIX] Context 0 uninitialized (FBW=0), falling back to context 1: "
           << "FBW=" << g_gs_state.frame[1].fbw
           << " SCISSOR=(" << g_gs_state.scissor[1].x0 << "," << g_gs_state.scissor[1].y0
           << ")->(" << g_gs_state.scissor[1].x1 << "," << g_gs_state.scissor[1].y1 << ")"
           << std::endl;
 ctx = 1;
}
...
auto& ti = job.batch.tex_info;
ti.enabled = g_gs_state.texture;
ti.fst = g_gs_state.use_uv;
if (ti.enabled) {
 // Copy TEX0 fields from active context
 ti.tbp  = g_gs_state.tex0[ctx].tbp;
 ti.tbw  = g_gs_state.tex0[ctx].tbw;
 ti.psm  = g_gs_state.tex0[ctx].psm;
 ti.tw   = g_gs_state.tex0[ctx].tw;
 ti.th   = g_gs_state.tex0[ctx].th;
 ti.tcc  = g_gs_state.tex0[ctx].tcc;
 ti.tfx  = g_gs_state.tex0[ctx].tfx;
 ti.cbp  = g_gs_state.tex0[ctx].cbp;
 ti.cpsm = g_gs_state.tex0[ctx].cpsm;
 ti.csm  = g_gs_state.tex0[ctx].csm;
  // Copy TEX1 fields
 ti.mmag = g_gs_state.tex1[ctx].mmag;
 ti.mmin = g_gs_state.tex1[ctx].mmin;
  // Copy CLAMP fields
 ti.wms  = g_gs_state.clamp[ctx].wms;
 ti.wmt  = g_gs_state.clamp[ctx].wmt;
 ti.minu = g_gs_state.clamp[ctx].minu;
 ti.maxu = g_gs_state.clamp[ctx].maxu;
 ti.minv = g_gs_state.clamp[ctx].minv;
 ti.maxv = g_gs_state.clamp[ctx].maxv;
  // Snapshot TEXA fields for alpha expansion
 ti.texa_ta0 = g_gs_state.texa.ta0;
 ti.texa_ta1 = g_gs_state.texa.ta1;
 ti.texa_aem = g_gs_state.texa.aem;
  // Snapshot VRAM region for texture data
 static constexpr size_t VRAM_SIZE = 4 * 1024 * 1024;
 static constexpr size_t PAGE_SIZE = 8192;
  int tex_w = 1 << ti.tw;
 int tex_h = 1 << ti.th;
  // Compute bytes per pixel based on PSM
 size_t total_bits = 0;
 switch (ti.psm) {
     case 0x00: // PSMCT32
         total_bits = (size_t)tex_w * tex_h * 32;
         break;
     case 0x01: // PSMCT24 (stored as 32bpp in VRAM)
         total_bits = (size_t)tex_w * tex_h * 32;
         break;
     case 0x02: // PSMCT16
         total_bits = (size_t)tex_w * tex_h * 16;
         break;
     case 0x13: // PSMT8
         total_bits = (size_t)tex_w * tex_h * 8;
         break;
     case 0x14: // PSMT4
         total_bits = (size_t)tex_w * tex_h * 4;
         break;
     default:
         // Fallback: assume 32bpp for unknown formats
         total_bits = (size_t)tex_w * tex_h * 32;
         break;
 }
  // Page-aligned snapshot size
 size_t total_bytes = (total_bits + 7) / 8;
 size_t pages_needed = (total_bytes + PAGE_SIZE - 1) / PAGE_SIZE;
 size_t snapshot_size = pages_needed * PAGE_SIZE;
  // Clamp to VRAM bounds
 if (snapshot_size > VRAM_SIZE) {
     snapshot_size = VRAM_SIZE;
 }
  ti.vram_snapshot_base = ti.tbp;
 uint32_t start = ti.tbp % VRAM_SIZE;
  if (start + snapshot_size <= VRAM_SIZE) {
     ti.vram_snapshot.assign(
         gs_vram.begin() + start,
         gs_vram.begin() + start + snapshot_size
     );
 } else {
     // Wraps around VRAM boundary
     ti.vram_snapshot.resize(snapshot_size);
     size_t first_part = VRAM_SIZE - start;
     std::memcpy(ti.vram_snapshot.data(), gs_vram.data() + start, first_part);
     std::memcpy(ti.vram_snapshot.data() + first_part, gs_vram.data(), snapshot_size - first_part);
 }
  // For indexed formats, also snapshot the CLUT region
 if (ti.psm == 0x13 || ti.psm == 0x14) {
     int clut_entries = (ti.psm == 0x13) ? 256 : 16;
     int bytes_per_entry = (ti.cpsm == 0x00) ? 4 : 2; // PSMCT32 or PSMCT16
     size_t clut_size = clut_entries * bytes_per_entry;
      ti.clut_snapshot_base = ti.cbp;
     uint32_t clut_start = ti.cbp % VRAM_SIZE;
      if (clut_start + clut_size <= VRAM_SIZE) {
         ti.clut_snapshot.assign(
             gs_vram.begin() + clut_start,
             gs_vram.begin() + clut_start + clut_size
         );
     } else {
         // Wraps around VRAM boundary
         ti.clut_snapshot.resize(clut_size);
         size_t first_part = VRAM_SIZE - clut_start;
         std::memcpy(ti.clut_snapshot.data(), gs_vram.data() + clut_start, first_part);
         std::memcpy(ti.clut_snapshot.data() + first_part, gs_vram.data(), clut_size - first_part);
     }
 }
  g_logFile << "[GIF] Texture snapshot: PSM=0x" << std::hex << ti.psm
           << " TBP=0x" << ti.tbp << " " << std::dec
           << tex_w << "x" << tex_h
           << " snapshot=" << ti.vram_snapshot.size() << " bytes" << std::endl;
  // TEXFLUSH: reset dirty flag after VRAM snapshot
 g_gs_state.texture_dirty = false;
}
// Snapshot RenderBatch-level GS register state
job.batch.colclamp = g_gs_state.colclamp;
job.batch.pabe = g_gs_state.pabe;
job.batch.fba = g_gs_state.fba[ctx];
job.batch.alpha_blend = g_gs_state.alpha_blend;
job.batch.fogcol = {g_gs_state.fogcol.r, g_gs_state.fogcol.g, g_gs_state.fogcol.b};
// Push to queue
g_renderQueue.Push(std::move(job));
// Clear local buffer
g_gs_state.draw_buffer.clear();
}
...
switch (reg) {
 ...
 // =====================================================================
 case GS_BITBLTBUF:
     g_logFile << "[GIF-XFER] A+D dispatch: reg=0x50 (BITBLTBUF) value=0x" << std::hex << value << std::dec << std::endl;
     g_logFile << "[GIF] REGLIST BITBLTBUF = 0x" << std::hex << value << std::dec << std::endl;
     g_gs_state.SetBitBltBuf(value);
     break;
 case GS_TRXPOS:
     g_logFile << "[GIF-XFER] A+D dispatch: reg=0x51 (TRXPOS) value=0x" << std::hex << value << std::dec << std::endl;
     g_logFile << "[GIF] REGLIST TRXPOS = 0x" << std::hex << value << std::dec << std::endl;
     g_gs_state.SetTrxPos(value);
     break;
 case GS_TRXREG:
     g_logFile << "[GIF-XFER] A+D dispatch: reg=0x52 (TRXREG) value=0x" << std::hex << value << std::dec << std::endl;
     g_logFile << "[GIF] REGLIST TRXREG = 0x" << std::hex << value << std::dec << std::endl;
     g_gs_state.SetTrxReg(value);
     break;
 case GS_TRXDIR:
     g_logFile << "[GIF-XFER] A+D dispatch: reg=0x53 (TRXDIR) value=0x" << std::hex << value << std::dec << std::endl;
     g_logFile << "[GIF] REGLIST TRXDIR = 0x" << std::hex << value << std::dec << std::endl;
     g_gs_state.SetTrxDir(value);
     break;
 case GS_HWREG:
     g_logFile << "[GIF-XFER] A+D dispatch: reg=0x54 (HWREG) value=0x" << std::hex << value << std::dec << std::endl;
 ...
 case GS_SIGNAL: {
     ...
     uint32_t idmsk = (value >> 32) & 0xFFFFFFFF;
     g_gs_regs.SIGLBLID = (g_gs_regs.SIGLBLID & ~((uint64_t)idmsk)) | ((uint64_t)(id & idmsk));
     g_gs_regs.GS_CSR |= 1;  // Set SIGNAL bit (bit 0)
     g_intc.RaiseInterrupt(INTC_GS);
     g_logFile << "[GIF] SIGNAL: ID=0x" << std::hex << id << " IDMSK=0x" << idmsk
               << " SIGLBLID=0x" << g_gs_regs.SIGLBLID << std::dec << std::endl;
     break;
 }
 case GS_FINISH:
     g_gs_regs.GS_CSR |= 2;  // Set FINISH bit (bit 1)
     g_intc.RaiseInterrupt(INTC_GS);
     g_logFile << "[GIF] FINISH: CSR=0x" << std::hex << g_gs_regs.GS_CSR << std::dec << std::endl;
     break;
 case GS_LABEL: {
     uint32_t id    = value & 0xFFFFFFFF;
     uint32_t idmsk = (value >> 32) & 0xFFFFFFFF;
     g_gs_regs.SIGLBLID = (g_gs_regs.SIGLBLID & ~((uint64_t)idmsk << 32)) | ((uint64_t)(id & idmsk) << 32);
     g_logFile << "[GIF] LABEL: ID=0x" << std::hex << id << " IDMSK=0x" << idmsk
               << " SIGLBLID=0x" << g_gs_regs.SIGLBLID << std::dec << std::endl;
     break;
 }
  // =====================================================================
 ...
 case GS_TEXFLUSH: // 0x3F
  ...
  case GS_DIMX: // 0x44
     ...
     break;
  case GS_PABE: // 0x49
     g_gs_state.SetPabe(value);
     g_logFile << "[GIF] REGLIST PABE = " << (value & 1) << std::endl;
     break;
  case GS_FBA_1: // 0x4A
     g_gs_state.SetFba(0, value);
     g_logFile << "[GIF] REGLIST FBA_1 = " << (value & 1) << std::endl;
     break;
 case GS_FBA_2: // 0x4B
     g_gs_state.SetFba(1, value);
     g_logFile << "[GIF] REGLIST FBA_2 = " << (value & 1) << std::endl;
     break;
 ...
 case GS_MIPTBP2_2: // 0x37
     ...
     break;
  case GS_TEX2_1: // 0x16
     g_gs_state.SetTex2(0, value);
     g_logFile << "[GIF] REGLIST TEX2_1 = 0x" << std::hex << value << std::dec << std::endl;
     break;
 case GS_TEX2_2: // 0x17
     g_gs_state.SetTex2(1, value);
     g_logFile << "[GIF] REGLIST TEX2_2 = 0x" << std::hex << value << std::dec << std::endl;
     break;
