...
void hle_sceSifBindRpc(CpuContext& ctx) {
    ...
    g_logFile << "HLE: sceSifBindRpc called" << std::endl;
    g_logFile << "     client_addr = 0x" << std::hex << client_addr << std::endl;
    g_logFile << "     server_id   = 0x" << server_id << " (" << server_name << ")" << std::endl;
    g_logFile << "     mode        = 0x" << mode << (mode & 1 ? " (non-blocking)" : " (blocking)") << std::dec << std::endl;
